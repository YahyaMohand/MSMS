    ...
    const newCommunityForm = () => (
        <div className='mt-3'>
            <ul className='list-group'>
                ...
                <hr></hr>
                {community.filter(name => name.com_name.toLowerCase().includes(values.toLowerCase())).sort((a, b) => a.com_name.localeCompare(b.com_name)).map((community, i) => (<CommunityCard key={i} community={community} />))}
            </ul>
        </div>
    )
    const printrecipt = () => {
        ...
        <div className='mt-5 mb-5 ml-3 mr-3'>
            <div className='row'>
                ...
                <div className="col input-group mb-2">
                    <label style={{ padding: '10px', fontWeight: 'bold' }} className="text-muted">Search Community Name :</label>
