    ...
    const newTrainingForm = () => (
        <div className='mt-3'>
            <ul className='list-group'>
                ...
                <hr></hr>
                {/* values === '' ? trainings.map((trainings, i) => (<TrainingCard key={i} trainings={trainings} />)) : */}
                {trainings?.filter(name => name.train_title.toLowerCase().includes(values.toLowerCase())).sort((a, b) => b.train_startdate.localeCompare(a.train_startdate)).map((trainings, i) => (<TrainingCard key={i} trainings={trainings} />))}
            </ul>
        </div>
    )
    const printrecipt = () => {
        ...
        <div className='mt-5 mb-5 ml-3 mr-3'>
            <div className='row'>
                ...
                <div className="col input-group mb-2">
                    <label style={{ padding: '10px', fontWeight: 'bold' }} className="text-muted">Search Training Name :</label>
