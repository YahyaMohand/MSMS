...
const AddSubCategory = () => {
     ...
     useEffect(()=>{
         axios.get(`${url}/admin/successstories/add/${userid}`)
         .then(res => {
    ...
    const {nameArabic, nameEnglish, categoryid, buttonText} = values

    const handleChange = (mainCategory) => (event) => {
        setValues({...values, [mainCategory]: event.target.value});
    }
    ...
    const clickSubmit = event => {
        ...
        axios({
            ...
            url: `${url}/admin/subcategories/create/${userid}`,
            data: {nameArabic, nameEnglish,  categoryid, logoPath}
        })
        .then(response =>{
            // console.log("Sub-Categories Added to database successfully", response);
            setValues({...values, nameArabic:'', nameEnglish: '', categoryid:'',logoPath:'', buttonText: 'Submitted'});
    ...
    return(
        <Layout>
            ...
            <div className="col-d-6">
                ...
                <h1 className="p-5 text-center">Add Sub-Category</h1>
